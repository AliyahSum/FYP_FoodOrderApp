    ...
    <body>
        <nav class="navbar navbar-expand-lg navbar-dark bg-dark fixed-top">
            <div class="container position-relative d-flex justify-content-center">
                ...
                <div class="navbar-brand">Kitchen Display System - Dessert</div>
            </div>
        </nav>
        ...
        <script>
            $(document).ready(function () {
                    ...
                    $('.serve-all-btn').click(function () {
                        ...
                        if (orderIDs.length > 0) {
                            if (confirm(`Are you sure you want to serve all orders for Table ${tableNo}?`)) {
                                $.post('updateOrderStatusDessert.php', { orderIDs: orderIDs, menuorderIDs: menuorderIDs }, function (response) {
                                    if (response.includes("successfully")) {
                ...
                function restoreCancelTextState() {
                    $('.dessert-name').each(function () {
                        const $this = $(this);
                        const menuorderID = $this.data('menuorder-id');
                        const isCanceled = localStorage.getItem(`dessert_cancel_${menuorderID}`);
                        if (isCanceled) {
                            $this.addClass('cancel-text');
                        }
                    });
