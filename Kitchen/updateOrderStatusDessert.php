...
if(isset($_POST['orderIDs']) && is_array($_POST['orderIDs']) && isset($_POST['menuorderIDs']) && is_array($_POST['menuorderIDs'])) {
    ...
    // Also update orderStatusDessert if needed
    $updateOrderStatusQuery = "UPDATE orders SET orderStatusDessert = 1 WHERE orderID IN ($orderIDsStr)";
    mysqli_query($link, $updateOrderStatusQuery) or die(mysqli_error($link));

    // Optionally return order IDs to JavaScript
