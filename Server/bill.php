...
if (!isset($_SESSION['usedTableSessions'])) {
    die("No Orders Yet :( <a href='tables.php'>Head Back</a>"); 
}
...
$recent_order_query = "SELECT orders.tableID, MAX(orders.orderID) as recentOrderID
                    FROM orders
                    GROUP BY orders.tableID";

$query = "SELECT menu_item.menuitemID, menu_item.item_name, SUM(menu_order.quantity) AS quantity, menu_order.special_request, menu_item.price
        FROM menu_item
...
if (empty($arrContent)) {
    die("No order data found for the specified table.");
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['paymentmethodID'])) {
    ...
    $orderID = isset($arrContent[0]['orderID']) ? $arrContent[0]['orderID'] : null;
